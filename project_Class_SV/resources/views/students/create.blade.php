        ...
        <form action="{{ route('students.store') }}" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="student_code" class="form-control" value="{{ old('student_code') }}" placeholder="Nhập mã SV" required>
                </div>
                    ...
                    <select name="class_id" class="form-select" required>
                        ...
                        @foreach ($classes as $classe)
                            <option value="{{ $classe->id }}" {{ old('class_id') == $classe->id ? 'selected' : '' }}>
                                {{ $classe->class_name }} ({{ $classe->class_code }})
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="address" class="form-control" rows="2" placeholder="Nhập địa chỉ thường trú">{{ old('address') }}</textarea>
            </div>
